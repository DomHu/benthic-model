...
!!$    call sub_closefile(loc_iou)
    ...
    DO i = 1,n_i
       DO j = 1,n_j
          if (sed_save_mask(i,j)) then
             ...
             call check_iostat(ios,__LINE__,__FILE__)
             write(unit=out,fmt='(A1,2A11,2A8,A8,A8,2A8,7A10,A10,A8,A10,A8,A10,5A10,12A10)',iostat=ios) &
                  & '%',                                          &
                  & '   time_kyr',                                &
                  & '  age_kyrBP',                                &
                  & '     lon',                                   &
  ...
  SUBROUTINE sub_sedgem_save_sedcoreenv( &
    ...
    call check_iostat(ios,__LINE__,__FILE__)
    write(unit=out,fmt='(1X,2f11.4,2f8.1,f8.1,f8.3,2f8.3,7f10.3,f10.3,f8.3,f10.3,f8.3,f10.3,5f10.3,12f10.3)',iostat=ios) &
  ...
  SUBROUTINE sub_data_save_seddiag_GLOBAL(dum_dtyr,dum_sfcsumocn)
    ...
    Write(unit=out,fmt=*) '% ----------------------------------------'
    Write(unit=out,fmt=*) '% '
    write(unit=out,fmt='(A1,2A4,2A8,3A8,5A10,2A8,4A10,8A10)',iostat=ios) &
         & '%',                                          &
         & '   i','   j',                                &
         & '     lon',                                   &
         & '     lat',                                   &
    ...
    DO i=1,n_i
       DO j=1,n_j
          IF (sed_mask(i,j)) THEN
             write(unit=out,fmt='(1X,2I4,2f8.1,3f8.1,5f10.1,2f8.3,4f10.1,8f10.3)',iostat=ios) &
                  & i,j,                                                     &
