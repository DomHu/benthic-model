...
!!$print*,'hello'
!!$stop
!!$print*,loc_new_sed(is_POC)/dum_dtyr,loc_new_sed(is_POC),dum_dtyr
!!$loc_wtpct = fun_sed_calcCorgwt(loc_new_sed(is_POC)/dum_dtyr,dum_D,0.8,2.7)
!!$print*,'fun_sed_calcCorgwt = ',tmp_var
!!$tmp_var = fun_arndtetal2013_sedpres(loc_wtpct,dum_D)
...
! NOTE: fun_arndtetal2013_sedpres calculates the fractional preservation of total Corg
       loc_sed_pres_fracC = fun_arndtetal2013_sedpres(loc_wtpct,dum_D)*loc_new_sed(is_POC)
       DO l=1,n_l_sed
          is = conv_iselected_is(l)
          if ( &
               & (sed_dep(is) == is_POC) .OR. &
