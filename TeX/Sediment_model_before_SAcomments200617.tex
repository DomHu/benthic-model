...
Equation \eqref{eq:ODE_OC} has the following general solutions depending on the bioturbation depth:
\begin{align}
\intertext{1. Layer ($z\leq z_{bio}$)}
...
\intertext{2. Layer ($z_{bio} < z < z_{ox}$)}
...
The integration constants ($A_{1,i},\ B_{1,i},\ A_{2,i}$) are derived by assuming a set of boundary conditions (Table \ref{Tab:BC_OM}). 
...
The steady-state solutions follow a simple exponential decay (see Eq. \ref{eq:GS_OM1} and \ref{eq:GS_OM2}) and drives the oxygen and nutrient dynamics. See Section \ref{subsec:GBCM} for further details 
on how to find the analytical solution.
...
Oxygen concentration is driven by the organic matter dynamics and can thus generally be described by:
\begin{align} 
...
\end{align}
and therefore results in two representations depending on the bioturbation depth:
\begin{align} 
\intertext{1. Layer ($z\leq z_{bio}$)}
...
\end{align}
with $D^1_{\chem{ \chem{O_2}}}$ and $D^2_{\chem{ \chem{O_2}}}$ being the \chem{O_2} diffusion coefficient for the bioturbated and non-bioturbated layer, respectively. 
The analytical solutions depending on bioturbation depth are given by: 
\begin{align}
\intertext{1. Layer ($z\leq z_{bio}$):}
...
\end{align}
To determine the four integration constants ($A_{O_2}^1,\, B_{O_2}^1,\, A_{O_2}^2,\, B_{O_2}^2$) and to solve the diagenetic equations a set of boundary conditions needs to be defined (see Table \ref{Tab:BC_O2}). 
...
The boundary conditions to solve Equations \ref{eq:NO3_ODE1_L1} - \ref{eq:NH4_ODE1_L2} are summarized in Table \ref{Tab:BC_NO3+NH4}. The analytical solutions are not further stated here but follow the same logic as for oxygen (for more details 
on how to find the analytical solution see Section \ref{subsec:GBCM}).
The model assumes known bottom water concentrations 
...
% \end{align}
with $D_{M}=D_{\mathrm{bio}}$ for $z\leq z_{\mathrm{bio}}$ and $D_{M}=0$ for $z > z_{\mathrm{bio}}$. 
...
Additionally it considers no change in phosphate flux and an asymptotic Fe-bound P concentration at $z_\infty$. \\
...
are taken from \citet{caroline_p_slomp_key_1996} \textcolor{red}{maybe change to \citet{palastanga_long_term_2011} as these are values used in a global model...}.
...
degradation rate constants for OM ($k_i$) are the most influential parameters and strongly determine the SWI-flux of redox-sensitive elements as well as the burial rate of OM. 
...
Here, we test globally invariant values for the OM degradation rate constants as well as the empirical relation of \citet{boudreau1997diagenetic}. Modelled 
mean POC content in the upper 5cm of the sediments are compared to the global POC observations in surface sediments (< 5cm sediment depth) of \citet{seiter_organic_2004}. 
...
%published parameterisations (see Table \ref{table:OM_degrad_para}) to test whether the different approaches are able to recreate main sediment characteristics. %importance of this parameter for our model results. 
