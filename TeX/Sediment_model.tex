...
Furthermore, organic matter degradation releases metabolic \chem{CO_2} to the pore water, causing it to have a lower pH and provoking the dissolution of \chem{CaCO_3} \citep{emerson_carbon_1981}.
...
rather than its variance only. The unconditional CDF, $F_y(y)$, of output $y$ is obtained when all uncertain parameters are varied simultaneously, and the conditional CDFs, 
$F_{y|x_i}(y)$, are obtained when all inputs but the $i$-th parameter are varied (i.e. $x_i$ is fixed to a so-called conditioning value). 
...
\begin{center}
	...
	B + C: Two examples of CDFs of the model calculated SWI-flux of \chem{NO_3}. 
	...
	at different fixed values for input parameter $k_1$ (B) and $K_{\chem{NH_4}}$, using n = 10 conditioning points. As the maximal distance between conditional CDFs 
	and unconditional CDF is greater for $k_1$ this parameter is more influentiaL for the model output (here SWI-flux of \chem{NO_3}, compare Fig. \ref{fig:Sensitivity_Analysis}). 
	}
...
We use NU = 200 samples to estimate the unconditional CDF, NC = 100 samples to estimate the conditional CDFs and n = 10 conditioning points, thus $200+100\cdot10\cdot11 = 11,200$ evaluations are performed for each sediment condition. 
...
\section{Coupled pre-industrial Earth system model simulations}
...
\end{equation}
for z=0. 
OMEN-SED then computes the fraction of POC preserved in the sediment at a depth of 100 cm (\chem{POC_{pres}}) and subsequently calls the routines to find the steady-state 
...
\subsection{Coupled pre-industrial simulations}\label{subsubsec:Methods_pre-ind_ESM_simul}
\subsubsection{Methodology}
...
\end{align}
where $x$ describes the relation between $k_1$ and $k_2$ and is subject to sensitivity experiments (with values of $x \in \{2, 4, 5, 8, 10, 25, 50, 100, 125\}$). 
...
\textbf{Dominik: Needs polishing, this is my first draft - also discussion of Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau} not done yet as results with new 
sedimentation rate might change slightly!}\\
...
\begin{center}
	...
	(\ref{boudreau_assumption2}) to calculate $k_1$ and $k_2$. Will be updated with new 20kyr runs and will make it prettier! Grid-points for more than 4.0 POC wt\% 
	are not shown.}
