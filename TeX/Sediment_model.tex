...
\begin{table}[tbp]
\caption{Reactions and variables implemented in the Reaction Network of OMEN-SED. The primary and secondary redox reactions are listed in the sequence they occur with increasing sediment depth.}
% title of Table
\centering
...
Here the coupling of OMEN-SED to the carbon-centric version of the ``GENIE'' Earth system model \citep[cGENIE][]{ridgwell_marine_2007} is described  (Fig. \ref{fig:OMEN-GENIE-coupling}). 
...
The ocean model is implemented on a 36$\times$36 equal-area horizontal grid with 16 vertical levels as in \citet{archer_atmospheric_2009}. 
...
Instead of completely remineralising POC at the seafloor, OMEN-SED is called by SEDGEM for each grid point. 
...
As nitrogen is not modelled explicitly in cGENIE the related stoichiometries in OMEN-SED are set to zero (i.e. \chem{NC_i}, $\chem{ALK}^\mathrm{NIT}$ and $\chem{ALK}^\mathrm{DEN}$). 
cGENIE, however, implicitly includes the effects of \chem{NH_4} release and its nitrification on Alkalinity but neglects the impact of P release; therefore, related 
...
Bottom water concentrations of solutes are converted from $\frac{mol}{kg}$ to $\frac{mol}{cm^3}$ and the depositional flux of POC (\chem{POC_{flux}}) is converted from $\frac{cm^3}{cm^2\,yr}$ 
to $\frac{mol}{cm^2\,yr}$ assuming an average density of POC of 1.0 cm$^{3}$ g$^{-1}$. 
...
The advection/sediment accumulation rate is generally taken from cGENIE, whereas a minimum values of $w=0.5\frac{cm}{kyrs}$ is imposed as OMEN-SED tends to be unstable for lower values. 
If the POC depositional flux is zero, OMEN-SED is not called and all SWI-fluxes of solutes and the fraction of POC preserved in the sediments (\chem{POC_{pres}}) are set to zero.
Otherwise, the bulk \chem{POC_{flux}} is seperated into the labile and refractory component and the routine to calculate the sedimentary POC profiles is called. 
Here, the two POC depositional fluxes are first converted into SWI concentrations (in $\frac{mol}{cm^3}$) by solving Eq. (\ref{Eq_flux_divergence}) for z=0.

OMEN-SED computes the resulting SWI-fluxes of solutes (in $\frac{mol}{cm^2\,yr^1}$) and the fraction of POC preserved in the sediment at a depth of 100 cm and returns the results to cGENIE. 
...
% For the solutes this is trivial; the OM flux is converted into a concentration (\chem{POC_{con}}, in $\frac{mol}{cm^3}$) using the sediment accumulation rate from cGENIE ($w$, in $\frac{cm}{yr}$) as:
% \begin{equation}
...
\textcolor{red}{TODO: Check e.g. Stolpovsky}
...
Figure \ref{fig:Sediment_profiles} compares profiles calculated with OMEN-SED to observations from different ocean depths taken at the Santa Barbara basin \citep[585 m, ][]{reimers_porewater_1996}, 
the Iberian margin (108 and 2213 m) and Naza\'e Canyon \citep[4298 m, ][]{epping_oxidation_2002}. 
Sediment-water interface characteristics and concentrations of total organic matter and all dissolved species in OMEN-SED were set to the observed values where available (Table \ref{table:Profiles_BC}). 
...
Phosphorus is adsorbed to \chem{Fe} oxides and incorporated into carbonate fluorapatite (\chem{CFA}) which is not modelled explicitly in OMEN-SED.  \textcolor{red}{above correct/okay....?}
...
Sensitivity Analysis (SA) is a term used for mathematical 
...
\textcolor{red}{TODO: Update description!}
The resulting indices (compare Fig. \ref{fig:SA_O2+NO3} in Appendix for the 4000m oxic sediment condition) 
...
\begin{table}[btp]
\caption{\textcolor{red}{Maybe in Suppl.} Model boundary conditions for the two idealised sediment conditions used for the sensitivity analysis 
(Fig. \ref{fig:Sensitivity_Analysis} and \ref{fig:SA_Color_ScatterPlots}). 
...
To test if OMEN-SED is able to reproduce observed SWI-fluxes, the Latin-Hypercube sampling outputs are compared with a global database of benthic fluxes of $\chem{O_2}$ and $\chem{NO_3}$ \citep{bohlen_simple_2012}. 
The coloured scatter plots (Figure \ref{fig:SA_Color_ScatterPlots}) show that the observed fluxes fall well in the range of SWI-fluxes calculated with OMEN-SED. Also highlighted by the emergence of colour patterns 
...
\begin{center}
	...
	\textcolor{red}{TODO:add text: Low influx, High influx...}}
	\label{fig:SA_Color_ScatterPlots}
	%>>>
\marginnote{\textbf{DH}: Why mainly negative NO3-flux 4000m?}[-1cm]%<<<
\end{center}
\end{figure}
...
\subsection{cGENIE pre-industrial}\label{subsec:GENIE-pre-ind}
\textcolor{red}{TODO: WRITE this section!}\\
We test different published parameterisations for the OM degradation rate constants by coupling OMEN SED to the pre-industrial cGENIE model setup. 
Model is run for 20,000 years to steady-state. OMEN-SED is called for each grid-cell in every timestep, feeding back to cGENIE the resulting SWI-fluxes and the fraction of 
POC preserved in the sediments. Describe Figure \ref{fig:GENIE_OMEN_Exps} and compare with what we know. E.g. Thullner et al. 2009! \\
