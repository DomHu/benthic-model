...
Represent the biggest, climate-relevant carbon pool in the surficial carbon cycle. Diagenetic processes represent an important link between carbon stored in more climate-relevant reservoirs like the oceans and the atmosphere, 
and carbon sequestered for much longer, geological timescales in sedimentary rock \citep{burdige2006geochemistry}. 
Highlight especially importance of OM burial \citep[see e.g.][]{burdige_preservation_2007, kriest_numerical_2011, arndt_quantifying_2013}.\\[2ex]
Diagenetic Models\\
...
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: added \chem{NH_4} adsorption here and in code ($K_\chem{NH_4}=1.3$)}}[-4cm]%<<<
\end{align}
% where
% \begin{align}
...
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: added $K_\chem{NH_4}$ here. (not in Gypens)}}[-5cm]
\marginnote{\textcolor{red}{\textbf{DH}: BC 5) for \chem{NH_4} should this not be $+(1-\gamma_{\chem{NH_4}})\cdot F_{\chem{NH_4}}$??}}[-2cm]%<<<
\end{table}
...
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: BC 5) don't delete $\int_{z_{\chem{\chem{SO_4}}}}^{\infty}$ here... profile would decreases then above zox}}[-13cm]%<<<
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: think correct, because at 8) \chem{CH_4} from $\int_{z_{\chem{\chem{SO_4}}}}^{\infty}$ is oxidised to \chem{H_2S}; at 5) this \chem{H_2S} to \chem{SO_4}}}[-10.5cm]%<<<
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: BC 5) for \chem{H_2S} should this not be $+(1-\gamma_{\chem{H_2S}})\cdot F_{\chem{H_2S}}$??}}[-3.5cm]%<<<
\end{table}
% % % % Sulfate:\\
...
\end{align}
%>>>
\marginnote{\textcolor{red}{\textbf{DH}: Added $K_{PO_4}$ in code. Why higher PO4 conc. even though diff-coeff is much lower now?}}[-4cm]%<<<
% where
% \begin{align}
...
Therefore, OMEN-SED (1.0) imposes a globally invariant bioturbation depth of 10cm. 
