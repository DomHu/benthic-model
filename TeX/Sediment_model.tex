...
for a generic tracer $C_i$ can be formulated as: % implemented in OMEN-SED
\begin{align} 
...
% with \xi: \frac{\partial \xi C_i}{\partial t} &= 0 = \xi D_i\frac{\partial^2C_i }{\partial z^2} - \xi w\frac{\partial C_i }{\partial z} + \xi (- k\cdot C_i - \sum_j \alpha_j \exp(-\beta_j z) + Q) \label{eq:generalODE}
%>>>
\marginnote{\textbf{DH}: What about $\xi$ in Eq. \ref{eq:generalODE}? Also include it here? Is this correct?}[-0cm]%<<<
\end{align}
where $k$ is a generic reaction rate constant % $t$ is time, $z$ is sediment depth, $D$ is the diffusion coefficient, $w$ is advection rate and 
...
%>>>
\marginnote{\textbf{DH}:More Info on these models given in Intro! which Vandenborght papers? - also ERSEM unclear (see Intro)}[-1cm]%<<<
%>>>
% {\small \textit{\textbf{DH: added to Intro: } Such analytical approaches with distinct layers were implemented and used in the eighties to describe observed pore water profiles 
% \citep[e.g.][]{billen1982idealized, goloway_diagenetic_1982} and later for inclusion into global Earth system models \citep{tromp_global_1995}. 
% However, these models in addition to the oxic layer these models only describe one anoxic layer, either a denitrification layer \citep{billen1982idealized, goloway_diagenetic_1982} or a sulfate reduction layer \citep{tromp_global_1995}.
% In addition, they ignore reoxidation of reduced species produced in the anoxic layers.}}
...
%>>>
\marginnote{\textbf{DH}: @ Sandra: Not sure what Bernie confused (flux of reduced sp. or general boundary matching) \& which paper it is: \citep{boudreau_early_1992, boudreau_kinetic_1992}!!??}[-1cm]%<<<
%>>>
