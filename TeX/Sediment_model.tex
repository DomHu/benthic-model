...
%>>>
\marginnote{\textbf{DH:} which Vandenborght papers? \& with ERSEM you mean the Plymouth model? Their model is vertically integrated...}[-1cm]%<<<
...
%>>>
\marginnote{\textbf{DH}: @ Sandra: Not sure what Bernie confused (flux of reduced sp. or general boundary matching) \& which paper it is: \citep{boudreau_early_1992, boudreau_kinetic_1992}!!??}[-0cm]%<<<
%>>>
...
Section \ref{subsec:globalhypsometry}. The modelled mean POC weight percent (wt\%) in the upper 5cm of the sediments is compared to the global distribution pattern of POC content in surface sediments 
(< 5cm sediment depth) of \citet{seiter_organic_2004}. 
Therefore, the original POC distribution pattern in $1^\circ \times 1^\circ$ grid resolution \citep[generated from > 5500 measurements, compare][]{seiter_organic_2004} has been transformed onto the $72\times 72$ SEDGEM grid 
(Figure \ref{fig:TOC_Obs_regridded}). \textcolor{red}{TODO: mention problem that we loose resolution, e.g. very high wt\% on the shelf} 
\textit{First, the original observations have been interpolated to generate a $1^\circ \times 1^\circ$ grid resolution \citep{seiter_organic_2004} which has subsequently been transformed onto the $72\times 72$ SEDGEM grid 
(Figure \ref{fig:TOC_Obs_regridded}). }
...
\textbf{Dominik: Needs polishing!}\\
OMEN-SED is coupled to the global Earth system model cGENIE as described in Section \ref{subsubsec:Methods_ESM_coupling}. The crossplots in this section (Figures \ref{fig:OMEN_GENIE_Boudreau_results} - \ref{fig:OMEN_GENIE_invariant_and_Boudreau}) 
...
\begin{center}
	...
	\caption{Crossplot comparing modelled and observed POC wt\% in the upper 5 cm of the sediments using the relationship of \citet{boudreau1997diagenetic} and the assumptions of Eq. (\ref{boudreau_assumption1}) and 
...
pattern of \citep{seiter_organic_2004} on to the SEDGEM grid, as some sites with higher POC wt\% on the continental margin are lost due to the restricted SEDGEM grid-resolution 
(compare e.g. maximum values for the East Pacific and upwelling waters of the Namibian shelf, Figure \ref{fig:TOC_Obs_regridded}). 
...
\begin{center}
	...
	\caption{TEXT TEXT: Will be updated with new 20kyr runs and will make it prettier!}\label{fig:OMEN_GENIE_invariant_results}
\end{center}
\end{figure}

\textbf{\textcolor{red}{DH: TODO go on from here!}} Compare different results... hopefully very different -> SA over k's is very important for global analysis! 
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/OMEN-GENIE-Exp/1406_COMPARE_CROSSPLOTS_INVARIANT_BOUDREAU.pdf}
	\caption{TEXT TEXT: TODO: Include other figures as in Fig. \ref{fig:GENIE_OMEN_Exps} here!}\label{fig:OMEN_GENIE_invariant_and_Boudreau}
\end{center}
\end{figure}
