...
For the solutes this is trivial and the OM flux is converted into a concentration (\chem{POC_{con}}, in $\frac{mol}{cm^3}$) using the sediment accumulation rate from GENIE ($w$, in $\frac{cm}{yr}$) as:
\begin{equation}
...
Other parameters needed GENIE the seafloor depth, the local temperature and the partitioning of bulk OM into the slower and faster degrading pool (and a third unreactive pool if modelled). 
...
In order to reduce reservoir requirements the sediment profiles (e.g. as shown in Fig. \ref{fig:Sediment_profiles}) are in general not calculated by OMEN-SED (1.0), however, the information needed to do this once the experiment 
