...
\textit{\textbf{DH: Include this discussion here:}
...
%>>>
\marginnote{\textbf{DH:} @ Sandra: which Vandenborght papers? \& with ERSEM you mean the Plymouth model? Their model is vertically integrated...}[-1cm]%<<<
...
\citep[hence, $D_{i,0}=D_{\mathrm{mol},i}\cdot f_{ir}$,][]{soetaert1996dynamic}. 
...
with their respective reaction stoichiometry. %Table \ref{table:Reaction_Network} summarises their reaction stoichiometry and Table \ref{table:ReactionOverview} provides an overview of their description in the model.
%>>>
...
Expressing Eq. (\ref{eq:Eq_generaldiagenetic}) for a set of chemical species thus results in a highly non-linear, coupled set of equations that can only 
...
($z_b \in \{z_{\mathrm{bio}}, z_{\mathrm{ox}}, z_{\chem{NO_3}}, z_{\chem{SO_4}} \}$). Its important to note that the boundaries of all biogeochemical zones 
(i.e. TEA penetration depths) are dynamically calculated and their depth thus varies in response to changing boundary conditions and forcings 
(see Section \ref{subsec:GBCM} for details). 
...
All consumption or production processes of dissolved species related to the degradation of organic matter are a function of the organic matter concentration and can thus be expressed 
as a series of exponential terms ($ \sum_j \alpha_j \exp(-\beta_j z)$, see Eq. (\ref{eq:generalODE})). In addition, adsorption and mineral precipitation 
processes can be expressed as zero or first order reaction ($Q$ or $k\cdot C_i$, in Eq. (\ref{eq:generalODE})). 
Fast adsorption is described as an instantaneous equilibrium reaction. 
...
reoxidation mainly occurs at an infinitesimally thin layer at the oxic/anoxic interface \citep{soetaert_model_1996}. 
...
$k$ is a generic first order reaction rate constant, $Q$ is a zeroth-order (or constant) reaction rate.
...
with 
\begin{equation} 
 a = \frac{w - \sqrt{w^2+4\cdot D\cdot k}}{2\cdot D}, \quad b = \frac{w + \sqrt{w^2+4\cdot D\cdot k}}{2\cdot D}
\end{equation}
where $A$ and $B$ are integration constants that can be determined by applying a set of internal boundary conditions (see Section \ref{subsec:module_Structure}).
% %>>>
...
flux at $z_\infty$. In addition, concentration and diffusive flux continuity across $z_{\mathrm{bio}}$ and $z_{\mathrm{ox}}$ is considered for \chem{NO_3} and \chem{NH_4}. 
...
boundary condition 8.2 (Table \ref{Tab:BC_SO4+H2S}). At the lower boundary $z_\infty$ zero flux of \chem{H_2S} is considered. 
...
to iron(III) hydroxides forming Fe-bound P \citep[or \chem{M},][]{slomp1998role}. Below the oxic zone,  \chem{PO_4} is not only produced via organic matter degradation but can also be released from the Fe-bound P pool due to the 
...
%>>>
\marginnote{\textbf{SA}: need to say something about carbonates here}[+1cm]%<<<
%>>>
...
% In this section we describe the methodology for our experiments to validate our model approach. 
% 
...
\subsection{Sensitivity Analysis}\label{subsec:methods_SA}
...
%>>>
\marginnote{\textbf{SA}: needs polishing \\
\textcolor{red}{\textbf{DH}: Better like this?}}[-0cm]%<<<
...
\subsection{Case study: Stand-alone simulations of global ocean transect}\label{subsec:globalhypsometry_methods}
%>>>
\marginnote{\textbf{SA}: needs polishing \\
\textcolor{red}{\textbf{DH}: Better like this?}}[-0cm]%<<<
%>>>
...
\subsubsection{Coupling to the cGENIE Earth system model}\label{subsubsec:Methods_ESM_coupling}
...
cGENIE, however, implicitly includes the effects of \chem{NH_4} release and its complete nitrification on alkalinity and neglects the impact of P release. Therefore, alkalinity 
...
OMEN-SED then computes the fraction of POC preserved in the sediment at a depth of 100 cm (\chem{POC_{pres}}) and subsequently calls the routines to find the steady-state solutions for the solute substances. 
Finally, \chem{POC_{pres}} and the SWI-fluxes of solutes (in mol\,cm$^{-2}$\,yr$^{-1}$) are returned to cGENIE. 
In case no POC is deposited on the seafloor (i.e. $\chem{POC_{flux}}=0$), OMEN-SED is not called and the SWI-fluxes of solutes and \chem{POC_{pres}} are set to zero. 
...
\begin{center}
	...
	and regridded observations onto the SEDGEM grid (right). Grid points without any observations are left blank (white, grey).}\label{fig:TOC_Obs_regridded}
\end{center}
\end{figure}

In section \ref{subsec:GENIE-pre-ind}, modelled mean POC weight percent (wt\%) in the upper 5cm of the sediments is compared to the global distribution pattern of POC content in surface sediments 
...
restricted SEDGEM grid-resolution (compare e.g. maximum values for the East Pacific and upwelling waters of the Namibian shelf, Figure \ref{fig:TOC_Obs_regridded}). 

To parameterise the reactivity of organic matter we test two different approaches. First, the empirical relationship proposed by \citet{boudreau1997diagenetic} is used (see also Section \ref{subsec:globalhypsometry_methods}), 
...
\begin{align}
  ...
  k_2 &= k_1/x					\label{boudreau_assumption2}
\end{align}
...
For the second approach, we assume globally invariant degradation rate constants. The rate constants for the more refractory OM pool, $k_2$, is systematically varied between 0.0002 and 0.005 year$^{-1}$ and the more labile 
OM component, described by $k_1$, is assumed to degrade 2, 4, 5, 8, 10, 20 and 50 times faster, respectively. 
...
of the corresponding reduced substance with \chem{O_2}, thus affecting the resulting SWI-fluxes. For the oxic setup, the reoxidation of \chem{H_2S} produced in the sulfidic layer ($\gamma_\chem{H_2S}$, Table \ref{Tab:BC_ALK} Eq. 5) 
also has a strong influence on alkalinity as it decreases alkalinity by 2 moles per mole of \chem{S} ($\chem{ALK}^\mathrm{H_2S}$, Table \ref{table:reaction_parameters}). 
For the anoxic setup the secondary redox parameters are essentially non-influential as no \chem{O_2} is available for the reoxidation of reduced substances. 
...
The sensitivities change if other $\chem{PO_4}$ related equilibrium concentrations $\chem{PO_4}^s$, $\chem{PO_4}^a$ and $\chem{M}^\infty$ are used (not shown).
...
%>>>
\marginnote{\textbf{SA:} needs a more critical analysis, what is the msg you want the reader to understand here?\\ \textbf{DH: How?}}[-1cm]%<<<\
...
% % \textbf{Sandra: needs more critical analysis\\
% % state that the model is able to capture main features across a wide range of different environments\\
% % -quckly explain the dynamics at each location- e.g. what are the dominant processes and how does OMEN do in capturing these?
% % -point out where it does well\\
% % -then point out where it does not soo well and explain why}
...
For the two open Iberian margin stations (108 and 2213 m) OMEN-SED fits all observations well. OMEN-SED does especially well at depth 2213 m by reproducing the deep \chem{O_2} penetration and the subsurface maximum in \chem{NO_3} 
concentration due to the nitrification of \chem{NH_4}. For the anoxic Santa Barbara Basin (585 m) the decrease in \chem{SO_4} and the increase in \chem{ALK} concentration with sediment depth is well represented, indicating the 
...
labile and refractory pool with different degradation rate constants. Thus, leading to a better fit of the \chem{NH_4} profile.  
...
% %%%%%% Info on specific sites:
% % \textit{Explanation later for high $H_2S$ in OMEN from Reimers (also check abstract):\\
% % Fe cycling exerts a strong control on the dissolved sulfide concentrations in the uppermost sediments of the SBB\\
% % for PO4: sediments become saturated with respect to carbonate fluorapatite and calcite within the first 0.25 mm and are highly supersaturated by and below 2 cm\\
% % from conclusion: for H2S and PO4: SBB conforms to a model of mineral paragenesis wherein CFA
% % accumulation is restricted to the near-surface of organic-rich
% % sediments. ...This finding is interpreted as an indication of the nonsteady state nature of the surface reactions...
% % In such surface sediments, a critical factor is that
% % iron oxyhydroxides are reduced, which raises pH and re-
% % leases sorbed phosphate, fluoride, and Fe*‘. The Fe*+ is then
% % available to sequester dissolved sulfide into FeS, leading to
% % pyrite formation. Also, colloidal precipitates of iron or sulfur,
% % or fresh bacterial products are suggested to supply surfaces
% % which may initiate CFA precipitation.\\
% % Generally, sulfide is undetectable in the uppermost 2-4 cm of sediment (detection limit 0.5 microM), and sulfide concentrations do not begin to
% % increase rapidly until depths where dissolved Fe is absent (Reimers et al., 1990). Porewater sulfide concentrations are below detection in spite of the presence of high rates of
% % sulfate reduction in the first O-2, and 2-4 cm of sediment (Fig. 5). In this zone, Fe is clearly being reduced and reacting with sulfide (primarily in the 2-4 cm interval) to form iron sulfides represented
% % as acid-volatile S and pyrite...\\
% % For the $PO_4$ minium - from Reimers: Ruttenberg and Bemer ( 1993) suggest diagenetic redistribution of organic P into CFA may be a nearly
% % continuous process at least to 50 cm in terrigenous dominated coastal sediments.}\\
% % 
% % \textbf{The NW Iberian Margin} ranks among the more productive areas of the global ocean (Longhurst, Sathyendranath, Platt, \& Caverhill, 1995)... The equatorward slope/shelf
% % current during summer is generally associated with the development of a band of cool upwelled North Atlantic Central Water (Fraga, 1981; Van Aken, 2001), which results there being high primary production
% % along the Iberian Margin.... The pattern of upwelling along the Iberian Margin is intermittent generating strong temporal variability in primary productivity and benthic deposition of carbon.
% % 
% % \textit{Site 108m - 99-06:\\
% % The concentrations of reduced iron and manganese at the sandy station were less, possibly the consequence of lower deposition of metal-oxides at this site}\\
% % 
% % \textit{Site 2213m - 99-04:\\ \textbf{could have been improved with a change of $\gamma_{NH_4}$}
% % The pore water profiles show that there was a much deeper penetration of oxygen and nitrate compared to the shelf stations,
% % whereas ammonium and dissolved iron and manganese did not accumulate above a depth of 12 cm. Con-
% % ceivably, carbon oxidation at this station is dominated by aerobic oxidation so that at this site denitrification
% % and anaerobic oxidation appear to be of minor importance. \textbf{Ammonium, released during organic matter
% % mineralization is effectively nitrified to nitrate as evidenced by the subsurface maximum in nitrate concentration, indicating a tight coupling between nitrification and denitrification at this station}.
% % }
% % 
% % \textit{Site 4298m (canyon) - 99-17:\\
% % At 4298 m, however, sandy layers are intercalated
% % with layers of high silt content, which is reflected in the erratic, but analytically highly reproducible distrbution of sediment organic carbon. These intercalations hint at the possibility of episodic deposition events
% % occurring over the fan of the canyon.\\
% % For the canyon, a problem arises because the oxygen microprofiles
% % were probably not being influenced by non-local exchanges because of the extremely small volume of pore
% % water that is sensed by the tip of the electrode, whereas the profiles of ammonium and nitrate are. Secondly,
% % OMEXDIA does not include a non-local transport routine\\ 
% % }
% % {\small probably not important: The accumulation of reduced manganese and iron
% % in the pore water as well as their declining concentrations down core, point to an intensive cycling of
% % metal-oxides and at a significant precipitation of sulfides and carbonates in these sediments}
% % 
% % \textbf{Overall need to consider that these are non-steady state conditions:}
% % (from Epping):
% % assuming steady state
% % conditions and, although the data were collected throughout the upwelling season, it will be considered to
% % represent yearly averages. Particularly for the Nazare Canyon and the shallow stations on the shelf, which
% % are subject to significant seasonality in physical and biological conditions, these assumptions may not be
% % valid.
% % and from Reimers: This finding is interpreted as an indication of the nonsteady state nature of the surface reactions
...
(red lines, see Section \ref{subsec:globalhypsometry_methods} for the methodology). 
Observations for \chem{O_2} and \chem{NO_3} fluxes are taken from \citet{middelburg_denitrification_1996}. Also plotted in Fig. \ref{fig:hypsometry}A are the total oxygen uptake (TOU) estimates presented in 
...
The 95\%-reoxidation experiments ($\bullet$, in Fig. \ref{fig:hypsometry}) show proportionally higher \chem{O_2} in-fluxes as the 5\%-reoxidation experiments ($\blacktriangledown$) because more \chem{O_2} is utilised for 
...
This is in line with results of \citet{thullner_global_scale_2009} who could show that in situ production is an important source for \chem{SO_4} and is responsible for $\sim$80\% of the total OM degradation rate at depths 
between 100 and 2000 m (\chem{SO_4} is not used for OM degradation in OMEN-SED below 2000m).
%>>>
\marginnote{\textbf{DH}: Not sure, if this 80\% is correct (seems to be pretty high)... but that's how I understand \citet{thullner_global_scale_2009} - see pg. 12 for 80\%!}[-1cm]%<<<
...
\textbf{Dominik: Needs polishing, this is my first draft - also discussion of Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau} not done yet as results with new sedimentation rate might change slightly!}\\
...
in order to see if a certain relation between $k_1$ and $k_2$ performs better for specific ocean depths, the data-points are binned into 6 uniform depth-classes of 1000 m each (respective mean POC wt\% and SFD are represented by 
...
\begin{center}
	...
	\caption{\textcolor{red}{TODO: Values might slightly change after updated experiments!} Crossplot comparing modelled and observed POC wt\% in the upper 5 cm of the sediments using the relationship of \citet{boudreau1997diagenetic} and the assumptions of Eq. (\ref{boudreau_assumption1}) and 
	(\ref{boudreau_assumption2}) to calculate $k_1$ and $k_2$. Will be updated with new 20kyr runs and will make it prettier!}
...
and $k_2$. In Figure \ref{fig:OMEN_GENIE_Boudreau_results} (A-J) the relation between the two degradation rate constants (Eq. (\ref{boudreau_assumption2})) is changed globally, thus independent of the seafloor depth. 
...
However, when looking at the individual bin-classes it can be seen that shallow ocean depths are better represented by smaller differences between $k_1$ and $k_2$ (e.g. $k_2 = k_1/4$, Figure \ref{fig:OMEN_GENIE_Boudreau_results}B),
and the deep ocean by a larger spread (e.g. $k_2 = k_1/100$, Figure \ref{fig:OMEN_GENIE_Boudreau_results}I). 
...
We use these observations to create a depth dependent relationship between the two degradation rate constants, where $x$ in Eq. (\ref{boudreau_assumption2}) takes values of 5, 10, 25, 50, 100 and 125 
for the 6 SFD bin-classes, respectively. The resulting regression model (Figure \ref{fig:OMEN_GENIE_Boudreau_results}K) accounts for 85.8\% of the variance of the modeled POC wt\% around the observed mean of the bin-classes. 
In addition, the slope of the regression line (1.0339) indicates that the rate of change in modeled and observed POC wt\% for the bin-classes is very similar.
%>>>
\marginnote{\textbf{DH}: more statistics, e.g. p-value don't make much sense right?}[-1cm]%<<<

However, imposing the depth dependent relation of the degradation rate constants is rather arbitrary and the POC wt\% data used for the comparison have their own limitations (compare Section\ref{subsubsec:Methods_pre-ind_ESM_simul}). 
...
Therefore, next we try to parameterise the degradation rate constants based on model intrinsic information. By providing two pools of POC from the water-column characterised by different degradation rate constants, cGENIE accounts 
...
double-accounting. Figure \ref{fig:OMEN_GENIE_invariant_results} presents results for the globally invariant degradation rate experiments, where $k_2$ is systematically varied between 0.0002 and 0.005 year$^{-1}$ and 
the more labile OM component, $k_1$, is assumed to degrade 2, 4, 5, 8, 10, 20 and 50 times faster, respectively. In general, 5 of the 6 bin-classes are located closer to the 1:1 line as in the previous experiments (with the exception 
of the shallow ocean bin-class). Also the slope of some regression lines is close to 1.0 (e.g.), indicating that this simple parameterisation captures the rate of change in modeled and observed POC wt\% for the bin-classes rather well. 
The shallowest bin-class (between 0 and 1000 m) represents an exception, as OMEN-SED tends to overestimate POC preservation for this depth class. However, this could also be related to the regridding of the original POC distribution 
pattern of \citep{seiter_organic_2004} on to the SEDGEM grid, as some sites with higher POC wt\% on the continental margin are lost due to the restricted SEDGEM grid-resolution (compare Section \ref{subsubsec:Methods_pre-ind_ESM_simul}). 
...
\begin{center}
	...
	\caption{\textcolor{red}{TODO: Values might slightly change after updated experiments!} 
	Results coupled cGENIE-OMEN-SED experiments using globally invariant degradation rate constants. }\label{fig:OMEN_GENIE_invariant_results}
\end{center}
\end{figure}

\textbf{\textcolor{red}{DH: TODO go on from here!}}\\

Describe Figure \ref{fig:OMEN_GENIE_invariant_and_Boudreau} and compare with what we know e.g.:\\
- zox increases from < 1 cm at 100 m; to > 10 cm at 5000m (Glud, 08; Meile \& Van Cappellen, 03)\\
- O2 gets more important as TEA with increasing SFD: <10\% at shallow SFD vs >80\% in the deep-sea (Thullner et al., 09)\\
- Cox decreases from the shelves to deep-sea: Total Cox rate is $\sim$50 times greater at 100m than at 5000m (Middelburg et al., 93; Thullner et al., 09; Schulz \& Zabel, 05)
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/OMEN-GENIE-Exp/0507_COMPARE_INVARIANT_BOUDREAU_allfigures.pdf}
	\caption{\textcolor{red}{TODO: Values might slightly change after updated experiments! }
	Results of OMEN-cGENIE coupling using representative parameterisations \citep[invariant and after][]{boudreau1997diagenetic} for the OM degradation rates ($k_1,\ k_2$, 
	compare Section \ref{subsubsec:Methods_pre-ind_ESM_simul}) All results shown are sediment characteristics calculated with OMEN-SED. }\label{fig:OMEN_GENIE_invariant_and_Boudreau}
\end{center}
\end{figure}
