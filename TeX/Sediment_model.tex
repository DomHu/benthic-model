...
In this section we test to which degree OMEN-SED is capable of capturing the dynamics of OM degradation pathways and related TEA-fluxes as simulated with a complete, numerical diagenetic model. 
...
second only 5\% (all other model parameters and boundary conditions are equal).
...
The corresponding fluxes of TEA into the sediments are shown in Figure \ref{fig:hypsometry} (D-F). Observations for nitrate fluxes, aerobic degradation, denitrification and sulfate reduction rates are taken 
from \citet{middelburg_denitrification_1996, middelburg_empirical_1997}. 


\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=0.6\textwidth]{figures/0_OMEN_Thullner_hypsometry_combined.pdf}
	\caption{(A-C) Carbon degradation rates along the global hypsometry and (D-F) fluxes of \chem{O_2}, \chem{NO_3} and \chem{SO_4} to the sediment. 
	Red lines are model results from \citet{thullner_global_scale_2009} using BRNS; black lines are results from OMEN-SED (solid: $\gamma_{NH_4}=\gamma_{H_2S}=0.95$; dashed: $\gamma_{NH_4}=\gamma_{H_2S}=0.05$). 
	Observations of carbon degradation rates (diamonds) are taken from \citet{middelburg_empirical_1997}, total oxygen uptake (TOU) estimates in Figure (F) use either the relationship 
	of \citet{andersson_respiration_2004} (TOU - observed) or are taken from \citet{thullner_global_scale_2009} (TOU - Thullner). 
