...
\affil[3]{Biogeosys, Department of Earth and Environmental Sciences, Universit\'e Libre de Bruxelles, Brussels, Belgium}
...
\textbf{I have slightly moved things around. There was some redundancy in the text regarding the paragraph concerning analytical diagenetic models and analytical models in ESM, as well as a part of the solution paragraph. 
It should flow nicely now, but go through the text and see if it is well integrated.}
...
CANDI, \citeauthor{boudreau_method--lines_1996}, \citeyear{boudreau_method--lines_1996}; MEDIA, \citeauthor{meysman_reactive_2003}, \citeyear{meysman_reactive_2003}; MUDS, \citeauthor{archer_model_2002}, \citeyear{archer_model_2002}; \textbf{OMEXDIA Soetaert 1995}; 
...
% lumped into one process, where degradation is only carbon limited; the terminal electron acceptors are not explicitly modeled, 
...
In marine sediments, particulate organic carbon (POC) is degraded by heterotrophic activity coupled to the sequential utilisation of terminal electron acceptors (TEAs) according to the free energy gain of the half-reaction 
...
\begin{align}
B_{1i} &\overset{\mathrm{BC 1)}}{=} \chem{POC}_{0i} - A_{1i} \notag\\
...
\intertext{For illustrative purposes, we here substitute the analytical solution for the POC depth profile and provide the analytical solution. The remaining paragraphs only outline the general equation, whose analytical solution can be derived in an identical manner. Substituting Eq. (\ref{eq:GS_OM1}) and (\ref{eq:GS_OM2}) for \chem{POC_i(z)} gives:}
...
\end{align}
where $D_\chem{\chem{ALK}}$ denotes the diffusion coefficient for alkalinity which depends on the bioturbation status of the respective geochemical zone. Note that the alkalinity description in the current version of OMEN-SED does not account for \chem{CaCO_3} dissolution/precipitation due to the mathematical complexity of the problem.
...
Equations (\ref{Solution_matchsoln}), (\ref{Solution_Upper_transformed_basis_fct}) and (\ref{Solution_NEW_EFG}) can now be consecutively applied for each of the dynamic biogeochemical zone boundaries (\ref{fig:Boundary_matching_algo}), starting at the bottom of the 
...
\textbf{we could also include the Burcwiz relationship as an option- it is more up-to-date, would be nice, virtually close to 0 work and shut up reviewers}
...
As the reoxidation of reduced substances produced during OM degradation may be incomplete, we allow part of the \chem{NH_4}, \chem{H_2S} and \chem{CH_4} flux to escape reoxidation. 
...
\textcolor{red}{More on that? REFERENCE where these values come from!!??} \textbf{they are just build in to allow for the escape of reduced species- see Gypens et al., 2008. there are no reference but evidence that some H2S, NH4 can escape oxidation. We assume an instantaneous re-oxidation at sox, but reality is that this is a reaction-transport problem. So if reaction timescales< transport timescales (e.g. a lot of competition for O2), some NH4 escapes. so this possibility is build in if wanted, e.g. observed. This is a parameter that just comes in because of the simplifying assumption of "everything is oxidised at zox"}
...
Model parameters implicitly account for processes that are not explicitly resolved. They are notoriously difficult to constrain and thus a primary source of uncertainty for numerical and analytical models- in particular on the global scale and/or in data-poor areas. 
...
\textbf{agree- as said before you should include more concluding statements- OM parametrisation is critical for these global models and gamma values (adsorption coefficients for different species too, but those are generally okieh constrained) can have very important influence on flux of reduced species through SWI. At the same time these are the weakest constrained parameters. Also get back to this point when discussing the transect -gamma- and the coupling- k. Gamma is probably a bit overrepresented here, because you allow it to vary between 0 and 1- basically all escapes to nothing. A more realistic range would be almost all- all. Thus gamma is introduced by the simplification and the deliberate relaxation of the simplification we allow for and then its role is overstressed because of its unconstrained nature. But one does not have to put up with this because you can also set it to "all consume" and assume that this is realistic enough.So gamma is only a switch include for cases where you can justify a relaxation of that assumption.OM degradation parameters are the holly grail! They are hugely important- one cannot overstate their importance, but very very poorly defined. The basic parametrisation of all other processes is not that critical, but it is vital to have a good handle on OM degradation parameters and think hard about gamma. So much care should be given to how these are parametrised in coupled simulations and I'd probably always recommend to run a range of different,plausible k- and gamma scenarios to quantify uncertainty.  }

% % \textbf{Sandra: also need to be careful how we formulate it, as you made k2 dependent on k1}. 
...
Because of the strong sensitivity of model results on OM degradation rate parameters, we further explore the sensitivity of simulated sediment-water exchange fluxes to variations in organic matter degradation parameters by varying $\chem{k_1}$, $\chem{f_1}$ and $\widetilde{\chem{k_2}}$ while all other model 
...
fluxes extracted from a global database \citep{stolpovsky_toward_2015} are indicated on the colour scale. The colour patterns in Figure \ref{fig:SA_Color_ScatterPlots} A and B also reveal the complex interplay between the amount of labile OM \chem{f_1} and its degradation rate \chem{k_1} for the resulting SWI-fluxes 
...
Figure \ref{fig:SA_Color_ScatterPlots} shows that SWI-fluxes can vary widely over the range of plausible organic matter degradation parameters. Simulated fluxes generally fall within the range of observed SWI fluxes. However, a large number of different k-f combinations can result in SWI that fall within the observed ranges reported by \citet{stolpovsky_toward_2015} further emphasising the care that should be devoted to constrain OM degradation parameters. 
...
that OMEN-SED is able to capture the main diagenetic features across a range of different environments without changing model parameters (other than the 3 we tuned \textbf{name them explicitly again!}) %(e.g. stoichiometric ratios or secondary reaction parameters) 
...
In general, Figure \ref{fig:hypsometry} shows that OMEN-SED captures the main trends in observed and numerically simulated TEA fluxes well. Results also confirm that higher gamma values better represent SWI for most of the global hypsometrie. A slight overestimation of shallow ocean SWI (water depth < 200m) for the high gamma scenario indicate that slightly lower gamma values would better capture SWI fluxes in these areas, where intense oxygen consumption processes and, thus rapid oxygen consumption favours the escape of reduced species across the SWI.
...
Depending on the configuration of the overlying biogeochemical ocean model, processes can be included or excluded in OMEN-SED and  stoichiometric factors (Tab. ??) need to be matched between models to ensure preservation of mass. 
As nitrogen is not modelled explicitly in the employed cGENIE configuration, \chem{NC_i}, $\chem{ALK}^\mathrm{NIT}$ and $\chem{ALK}^\mathrm{DEN}$ in OMEN-SED are set to zero. 
...
Within the water column in cGENIE, POC is partitioned into two fractions with different degradation length scales of ?? and ??, respectively. The labile pool thus degrades while sinking through the water column, 
...
\end{equation}
In case OMEN-SED computes unrealistic results for POC preservation (i.e. $f_{\mathrm{POC}} < 0.0$ or $f_{\mathrm{POC}} > 1.0$) we discard the results of OMEN-SED and all POC is remineralised at the ocean floor. \textbf{I'd add a sentence to indicate how frequent that is}
...
Furthermore, when OMEN-SED is coupled to cGENIE, very different timescales have to be considered for OM degradation in the sediments compared to the water column (Fig. \ref{fig:OM_reactivity}A+B) and thus the diagenetic rate constants cannot be easily implied by the assumed water column POC flux profiles in cGENIE. 
 To illustrate this, lets consider the degradation of fresh, marine organic matter as it transported and degraded along the ocean-sediment continuum. The bulk material is composed of a complex mixture of different organic carbon compounds that can be described by a reactivity continuum. Microbes preferentially degrade the more reactive organic matter compounds first \citep{emerson_processes_1988, wakeham_compositions_1997, lee_composition_2000}, resulting in the prefferential preservation of more unreactive compounds, rendering the remaining mixture less and less reactive with time. Thus depending on the age of OM (or depth in the sediment and water column) the 
reactivity distribution of its compounds changes significantly (Fig. \ref{fig:OM_reactivity}) and the multi-G (2G in this case) approximation of this continuum has to take this shift into account. Fig. \ref{fig:OM_reactivity} illustrates these changes in the original reactivity distribution within a ocean-sediment framework. The reactivity distribution $t<1$ year represents the organic matter mixture after it settled through the water column (Fig. \ref{fig:OM_reactivity}C). Only the most reactive OM compounds are remineralised. This explains why the POC flux in the ocean can be represented with a 1G or pseudo 2G degradation model. In the sediments however much longer timescales have to be considered and a wider range of more unreactive compounds are degraded. As a consequence, significant changes in the reactivity distribution already take place in the upper mm of the sediments ($t \sim 10$ years, Fig. \ref{fig:OM_reactivity}C). Therefore, a broader range of OM reactive types must be represented by the degradation model to capture the reactivity spectrum of OM in surface sediments, explaining why at least a pseudo 3G model  \citep[including two degradable and one refractory fraction][]{soetaert_model_1996, boudreau1997diagenetic, stolpovsky_toward_2015}
To complicate the situation even further, different sediment depths can represent very different timescales. For instance, half a meter of sediment can be deposited within a year in a coastal setting, while it will represent thousands of years (if not more) of sedimentation in a deep ocean setting. Residence times and thus degradation rate timescales (or OM age) is mainly controlled by advection rates. For instance, assuming an advection rate of 0.01 cm/yr for the shallow ocean, OM at 5cm depth is has been degraded for approximately 500 years, whereas a deep ocean advection rate of 0.001 cm/year allowed for OM degradation of approximately 1000 years at the same depth. As a consequence, organic matter degradation in deep ocean sediments affects a much wider range of the reactivity continuum and our simple pseudo 3G approximation of the complex OM mixture needs to reflect this by allowing for different k and f values (Fig. \ref{fig:OM_reactivity}C).  
...
Nevertheless, these relationships are generally based and/or tested on limited data sets and their global applicability, especially for under past or projected future environmental conditions is questionable \citep{arndt_quantifying_2013}. 
We hence test several alternative schemes in the coupled OMEN-cGENIE framework. 
Our objective is not to perform and discuss a detailed calibration of the coupled models as this is beyond the scope of this sediment model development paper. Rather we want to showcase the feasibility of the model coupling, illustrate the range of results and thus information that can be generated with OMEN-SED and verify that model results capture the main observed global benthic biogeochemical features. 
...
For the spatially uniform experiment A ($(k_2, x) = (0.005, 1.3)$) modelled SWI-fluxes \textbf{why not plot the O2 observations from the global hypsometry figure and/or stopolsky as dots on the map? Or you could also show the Seiter ODU map for oxic degradation rates. I also have seen globals maps of O2 consumption- will try to digg this out. Should be quickly done as all the data is compiled.}and sediment characteristics are shown in Figure \ref{fig:OMEN_GENIE_best_fit_invariant}.
...
This is in agreement with data from the literature \citep[e.g.][]{middelburg_organic_1993, middelburg_empirical_1997} \textbf{make also reference to Burdige figure in his book, but probably also in the 2007 paper. if you cannot find it ask Philip to send you a photo of it. He has got the book} and other model results 
...
State-of-the art numerical models representing the full complexity of the diagenetic processes typically perform well at reproducing site-specific biogeochemical dynamics. However, tuning model parameters is laborious, 
...
However, the model presented here, despite being more complete than previous analytical models, is inevitably associated with a certain degree of simplifications. In order to solve the diagenetic equation analytically important assumptions have been made, which limit 
...
or bottom water oxidation state on degradation and burial dynamics, for instance during OAEs. Yet, OMEN-SED is not able to predict the system response to short-term or seasonal variations of boundary conditions. In future versions, non-steady state conditions could be approximated by implementing a relaxation between different steady states, similar to the schemes used in \textbf{cite Arndt and Regnier, 2007 and Rujardij 1995}, thus enabling an application of the model to system characterised by higher variability in boundary conditions. In addition, the unavoidable assumption of depth invariant porosity limits the correct calculation of the sediment-water interface flux of dissolved species as in reality porosity decreases with sediment depth. \textbf{but how important is this??} \\

In addition, the global parametrisation of the sensitive model parameters (OM degradation and secondary redox gamma) is not straight forward. A comparison between simulated OM contents and observations indicates that depth dependent k-f relationships provide the best fit (section ??), confirming more theoretical considerations regarding the different time and reactivity scales that need to be considered. With respect to gamma values, model simulations along the global hypsometry (section ??) have shown that high gamma values generally capture the main flux features, but have also highlighted that slightly lower gamma values would result in a better fit of SWI to observations. In the future, fully formulated, multi-component diagenetic models could be used to derive transfer functions that would allow quantifying gamma values as a function of environmental forcings, such as organic matter degradation rates. \textbf{check Gypens at all. do they discuss this???}
Currently, no general theoretical framework exists to quantify these parameters on a global scale. As a consequence, attention should be devoted to choosing their values and sensitivity studies over plausible ranges are recommended.


Furthermore, the current version of OMEN-SED does not yet resolve the full range of important biogeochemical species, thus limiting its applicability.  Manganese and iron dynamics are not included and as such OMEN-SED is not able to simulate all processes important in coastal marine environments and highly accumulating upwelling regions. Furthermore, especially the lack of iron, limits the scope of applicability of the model, especially in systems characterized by high Fe oxide inputs and under anoxic conditions, where redox driven iron dissolution-precipitation reactions induce a high internal recycling of the sedimentary Fe pool. This can cause problems when modelling \chem{H_2S} and \chem{PO_4} profiles in anoxic environments as their concentrations are affected by these metal ions (compare Section \ref{subsec:SedProfiles}). Therefore, already planned future work will focus on an explicit description of iron. 

Finally, although the model also simulates DIC and alkalinity production and, thus, has the potential to predict pH profiles within the sediment, major limitation at this stage is the lack of an explicit description for \chem{CaCO_3} precipitation/dissolution coupled to OM decomposition, which also controls the inorganic carbon system  \citep{krumins_dissolved_2013}. 
...
as well as the reoxidation of reduced substances produced during organic matter degradation. Pore water tracers include \chem{O_2}, \chem{NO_3}, \chem{NH_4}, \chem{SO_4}, \chem{H_2S}, \chem{DIC} and \chem{ALK} and the solid phase includes two degradable fractions of organic matter, Fe-bound P and authigenic Ca-P minerals. \\
An extensive sensitive analysis, based on the novel density-based PAWN method \citep{pianosi_simple_2015} emphasises the key role of OM degradation rate parameters and thus highlights the need for the development f for th are the most sensitive parameter. In addition, under anoxic conditions secondary redox parameters exert an important control on related SWI-fluxes of \chem{SO_4}, \chem{H_2S}, \chem{NH_4} and alkalinity. We have shown that the performance of OMEN-SED is similar to that of a fully formulated, multi-componentn numerical model and that  the new analytical model is able to reproduce observed pore water profiles across very different depositional environments and captures observed large scale patterns of surface sediment organic matter contents \citet{seiter_organic_2004} as well as SWI-fluxes, oxygen penetrateion depths and biogeochemical reaction rates. 
Furthermore, as the major parts of the global carbon cycle are included in the new model it is well suited to examine the role of sediments for global biogeochemical cycles and climate and for exploring feedbacks within the Earth system in response to a wide range of perturbations and over various timescales. 
