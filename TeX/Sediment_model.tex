...
Physical and chemical (or diagenetic) processes in sediments depend on the water column and vice versa: Diagenesis is mainly donor controlled, as it is fuelled by the external supply of solid material 
...
However, the overall amount, exact timing and the rate of organic matter burial remain a topic of intense debate. 
...
MEDUSA, \citeauthor{munhoven_glacialinterglacial_2007}, \citeyear{munhoven_glacialinterglacial_2007}). In particular, these models consider fewer biogeochemical reactions and assume that the sedimentary organic matter pool is composed 
of just a single or two discrete compound classes. % apply the so-called 1G approach, thus just representing a single organic matter pool which is degraded at a constant rate. 
\textcolor{red}{TODO: rest somewhere else} 
Obviously, such a simplification can neither account for the observed vast structural complexity in natural organic matter and its resulting 
different degradation rates \textit{nor for the decrease in OM degradability during diagenesis - holds just for 1-G} \citep{arndt_quantifying_2013}. 
...
However, also these relationships are mostly based on simple fitting exercises to limited data sets and cannot replace a mechanistic representation of the underlying processes, especially when applying the model 
for different geological timescales. \\[2ex]
...
This approach thus ignores the vertical extent of the sediments and the temporary storage of dissolved species \citep{soetaert_coupling_2000}. However, it is computationally efficient and 
also allows differentiating between various fractions of organic matter. Most EMICs incorporate a vertically integrated dynamic model for particulate inorganic carbon only (i.e. mainly \chem{CaCO_3}) and just a few are 
considering oxic-only sediment degradation of organic matter \citep{huelse_biopump_models_2016}. 
...
PISCES \citep{gehlen_reconciling_2006} and CLIMBER 3$\alpha$ \citep{ilyina_global_2013}, both using a version of \citet{heinze_global_1999}. % and NORESM \citep{tjiputra_evaluation_2013} does not say anything if they just use the original oxic-only
None of the reviewed EMICs in \citet{huelse_biopump_models_2016} uses such a sediment representation. 
...
\textcolor{red}{TODO:} How they usually quantify the rate of OM degradation (see Stolpovsky et al. 2015 or/and \citep{soetaert_coupling_2000, mackenzie_past_2004}.\\
...
\textbf{Alternative Model approaches} e.g. from coastal research\\[2ex]
