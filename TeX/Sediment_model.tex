...
\citep[e.g.][]{billen1982idealized, goloway_diagenetic_1982} \textcolor{red}{\textbf{which Vandenborght papers?}} and later for inclusion into global Earth system models \citep{tromp_global_1995}. 
...
the dissolved and solid species explicitly resolved in OMEN-SED. Tables \ref{table:reactions_processes} and \ref{table:Reaction_Network} provide a summary of the reactions and biogeochemical tracers considered in OMEN-SED, as well 
as their respective reaction stoichiometry. %Table \ref{table:Reaction_Network} summarises their reaction stoichiometry and Table \ref{table:ReactionOverview} provides an overview of their description in the model.
%>>>
...
and $g(z) = -\alpha_j \exp(-\beta_j z) + Q$ describes the inhomogeneous reaction (production/consumption) of tracer $C_i$, where $Q$ is a zeroth-order (or constant) rate of $C_i$ reaction, 
...
%In order to facilitate the analytical solution and in addition to assuming steady-state 
...
Each of these zones is characterised by a different set of equations that encapsulate the most pertinent reaction and transport processes within the respective zone in a linear form (see Section \ref{subsec:ReactionNetwork} for 
more details), thus allowing a simplification of the coupled set of equations in order to find an analytical solution \citep[compare e.g. ][for similar solutions]{billen1982idealized, goloway_diagenetic_1982, tromp_global_1995}. 
%>>>
\marginnote{\textbf{DH}: \textcolor{red}{\textbf{which Vandenborght papers?}}}[-1cm]%<<<
%>>>
...
\textbf{TODO from here: explain: \\
solve OM equation first, express most processes as a function of OM ...\\}
\textcolor{red}{Describe what we do with diffusive flux at boundaries... in order to get the analytical solution} as has been done in \citep{boudreau_early_1992}
%>>>
\marginnote{\textbf{DH}: @ Sandra: I think, flux in internal boundaries as in \citep{boudreau_early_1992}!!??}[-1cm]%<<<
%>>>

Based on Eq. (\ref{eq:Eq_generaldiagenetic}) and its analytical solution Eq. (\ref{eq:ODE_general_solution_generalapproach}), OMEN-SED calculates the fraction of particulate organic carbon (POC) buried in the sediment, 
...
To validate our model approach an extensive sensitivity analysis for the most important model parameters is performed with the stand-alone version and resulting sediment-water interface fluxes are compared with a global 
database (Section \ref{subsec:SA}). 
...
output distribution \citep[e.g. variance-based or density-based approaches][]{pianosi_sensitivity_2016}. Especially the latter indices are easy to interpret and can be compared across different parameters and/or different model 
outputs as 
...
%>>>
Section \ref{subsec:SedProfiles} compares modelled profiles with measured pore water data from different ocean depths including the continental shelf (108\,m), upper slope (585\,m), lower slope (2213\,m) 
