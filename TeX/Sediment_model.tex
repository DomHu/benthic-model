...
When looking at the individual bin-classes it can be seen that shallow ocean depths are better represented by smaller differences between $k_1$ and $k_2$ (e.g. $k_2 = k_1/2$ for SFD < 1000m, Figure \ref{fig:OMEN_GENIE_Boudreau_results}A),
and the deep ocean by a larger spread (e.g. $k_2 = k_1/25$ for SFD > 3000m, Figure \ref{fig:OMEN_GENIE_Boudreau_results}F). 
...
However, imposing the depth dependent relation of the degradation rate constants is rather arbitrary and the POC wt\% data used for the comparison has their own limitations (compare Section\ref{subsubsec:Methods_pre-ind_ESM_simul}). 
...
pattern of \citep{seiter_organic_2004} on to the SEDGEM grid, as some data grid-cells with higher POC wt\% on the continental margin are lost due to the restricted SEDGEM resolution (compare Section \ref{subsubsec:Methods_pre-ind_ESM_simul}). 
...
\citet{seiter_organic_2004} data better than a larger spread between both POC pool (i.e. $x \in \{4, 5, 10\}$).

\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/OMEN-GENIE-Exp/0_1507_Invariant_k_CROSSPLOT_WITH_REGRESSION.pdf}
...
Figure \ref{fig:OMEN_GENIE_invariant_and_Boudreau} compares results from experiments using globally invariant degradation rates ($(k_2, x) \in \{(0.005, 3), (0.007, 10)\}$) 
with approaches using the relation of \citet{boudreau1997diagenetic} ($k_2 = k_1/5$ and depth dependent). 
Global difference maps of model and \citet{seiter_organic_2004} surface sediment POC wt\% are compared and sediment characteristics related to organic matter 
degradation and oxygen consumption calculated in OMEN-SED are shown. 
...
All parameterisations underestimate mean POC wt\% in the surface sediments of the equatorial east Pacific and overestimate POC concentrations in 
the North Pacific and Southern Oceans (Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau}, 2nd row). 
...
For all 4 experiments modelled total POC degradation (\chem{POC_{degr}}) rates in the upper 5cm decrease from the shelves to the deep sea by up to almost 2 orders of magnitude 
(Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau}, 3rd row)
which is in agreement with data from the literature \citep[e.g.][]{middelburg_organic_1993, more} and other model results \citep[e.g.][]{thullner_global_scale_2009, more} 
indicating that the highest degradation rates in marine sediments are to be found on the continental margins. 
The fraction of aerobic POC degradation in the upper 5cm increases from the shelves to the deep sea (Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau}, 4th row) 
which is also consistent with data compilations \citep{see_thullner:Middelburg} as well as other model results. \citet{thullner_global_scale_2009} for example 
concluded that oxygen is responsible for less than 10\% of \chem{POC_{degr}} at 100m SFD and for more than 80\% in the deep sea. 

The oxygen penetration depth increases from below 1cm at the shelves to more than 10cm in the deep ocean (Fig. \ref{fig:OMEN_GENIE_invariant_and_Boudreau}, last row)
... also evidence for very deep O2 penetration in deep sea sediments! (r is thisd also in here: Glud, 08; Meile \& Van Cappellen, 03)
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=1.0\textwidth]{figures/OMEN-GENIE-Exp/0_1507_1707_COMPARE_INVARIANT_BOUDREAU_allfigures.pdf}
