...
Determining the four integration constants ($A_{\chem{O_2}}^1,\, B_{\chem{O_2}}^1,\, A_{\chem{O_2}}^2,\, B_{\chem{O_2}}^2$, see Section \ref{subsec:GBCM} for details), as well as the $a-priori$ unknown oxygen penetration 
...
of $\chem{NH_4}$ and $\chem{H_2S}$ diffusing into the oxic zone from below (BC 4.2, Table \ref{Tab:BC_O2}). It is assumed that respective fractions ($\gamma_{\chem{NH_4}}$ and $\gamma_{\chem{H_2S}}$) are directly re-oxidised 
...
In addition, bottom water conditions are changed to low oxygen/high nitrate levels more likely to be found in the Pacific Ocean ($\chem{O_2} = 10$ nanomole cm$^{-3}$ and $\chem{NO_3} = 80$ nanomole cm$^{-3}$). 
...
\begin{center}
	...
	\caption{Global distribution of TOC observations (N=5500) from the database of \citet[][left]{seiter_organic_2004} and regridded observations onto the SEDGEM grid (right). Grid points without any observations are left blank (white). 
...
Here, we test globally invariant values for the OM degradation rate constants ($k_1, k_2$) as well as the empirical relation of \citet{boudreau1997diagenetic} which has also been used in Section \ref{subsec:globalhypsometry}. 
The modelled mean POC content in the upper 5cm of the sediments is compared to the global POC observations in surface sediments (< 5cm sediment depth) of \citet{seiter_organic_2004}. 
The original data field of \citet{seiter_organic_2004} has been transformed onto the $72\times 72$ SEDGEM grid (Figure \ref{fig:TOC_Obs_regridded}). % in order to compare model results with the observations.
...
\begin{align}
  ...
  k_2 &= k_1/x
\end{align}
where $x$ describes the relation between $k_1$ and $k_2$ and is varied in our sensitivity experiments (compare Figure \ref{fig:OMEN_GENIE_Boudreau_results}). 
As the fractions of labile and refractory OM ($f_1, f_2$) reaching the sediments is known we can calculate $k_1$ and $k_2$ independtly for each grid-cell. 

For the globally invariant degradation rate experiments $k_2$ is systematically varied between 0.0002 and 0.005 year$^{-1}$ and the more labile OM component, described by $k_1$, is assumed to degrade 2, 4, 5, 8, 10, 20 and 50 times 
faster, respectively. 
. 
%The original data field is interpolated on a $1^{\circ}\times 1^{\circ}$ grid and has been transformed onto the $72\times 72$ SEDGEM grid (Figure \ref{}).
