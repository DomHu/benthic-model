...
OMEN-SED has been coupled to the global Earth system model cGENIE as described in Section \ref{subsubsec:Methods_ESM_coupling}. Figure \ref{fig:OMEN_GENIE_Boudreau_results} compares POC wt\% from the regridded global distribution 
pattern of \citet{seiter_organic_2004} with modelled POC wt\% using the relationship of \citet{boudreau1997diagenetic} and the assumptions of Eq. (\ref{boudreau_assumption1}) and (\ref{boudreau_assumption2}) to calculate $k_1$ 
and $k_2$. The colour of the points represents the SFD of the respective cGENIE grid-cell. As the individual data-points are highly scattered and in order to see if a certain relation between $k_1$ and $k_2$ performs 
better for specific ocean depth, the data-points are binned into 6 uniform depth-classes of 1000 m each (respective mean POC wt\% and SFD are represented by the triangles). The regression line (and the corresponding R$^2$-value) is 
calculated for the 6 bin-classes and included in the figures. \\
\textbf{TODO: from here: }
First, the relation between the two degradation rate constants (compare Eq. (\ref{boudreau_assumption2})) is changed globally, thus independent of the seafloor depth (Figure \ref{fig:OMEN_GENIE_Boudreau_results} A-J). 
The crossplots show that it is not possible to achieve a solution where all bin-classes fall onto or close to the 1:1 line. However, if looking at the individual bin classes it can be seen that shallow ocean depths are better 
represented by smaller differences between $k_1$ and $k_2$ (e.g. $k_2 = k_1/4$), and the deep ocean by a larger spread (e.g. $k_2 = k_1/100$). This can be interpreted, in a greatly simplified way, as OM in the shallow ocean 
being fresher and more reactive, therefore the degradation rate constants are more similar. 
...
\begin{center}
	...
	(\ref{boudreau_assumption2}) to calculate $k_1$ and $k_2$. }
