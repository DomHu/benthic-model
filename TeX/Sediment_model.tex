...
the dissolved and solid species explicitly resolved in OMEN-SED. Tables \ref{table:reactions_processes} and \ref{table:Reaction_Network} provide a summary of the reactions and biogeochemical tracers considered in OMEN-SED. 
\textbf{SA: Tab 1 is maybe a bit redundant? DH: I still think it's a good summary/overview!? and the other one could go into the appendix!?} 
...
produced in the anoxic zones of the sediment (here all zones $z < z_{\mathrm{ox}}$ combined). In the oxic zone ($z<z_{ox}$), the aerobic degradation consumes oxygen with a fixed \chem{O_2:C} ratio (\chem{O_2C}, Tab. \ref{table:reaction_parameters}). 
...
of $\chem{NH_4}$ and $\chem{H_2S}$ diffusing into the oxic zone from below.  It is assumed that respective fractions ($\gamma_{\chem{NH_4}}$ and $\gamma_{\chem{H_2S}}$) are directly re-oxidised at the oxic/anoxic interface. 
...
where $D_\chem{SO_4}$ and $D_{\chem{H_2S}}$ denote the diffusion coefficients for \chem{SO_4} and \chem{H_4S} which depend on the bioturbation status of the respective geochemical zone (compare Section \ref{subsec:GBCM}). 
...
reduction of iron-hydroxides under anoxic conditions. Furthermore, in these zones phosphate concentrations build up and pore waters can thus become over-saturated with respect to apatite, thus triggering the authigenic formation of 
...
The solution of Eq. (\ref{eq:generalODE}) is of the general form:
\begin{align}
...
\end{align}
where $E(z)$, $F (z)$ are the homogeneous solutions of the ODE, G(z) the particular integral, and A, B are the integration constants that must be determined with the boundary conditions 
...
\textcolor{red}{TODO: Go on from here!}
...
%are consumed during nitrification of \chem{NH_4} produced in the anoxic zone. As nitrification may be incomplete in the oxic zone, we similarly allow for part of the ammonium flux to escape re-oxidation. 
\begin{table}[btp]
