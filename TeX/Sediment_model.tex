...
\subsection{Case study: Stand-alone simulations of sediment cores}
%>>>
\marginnote{\textbf{SA}: needs polishing \\
\textcolor{red}{\textbf{DH}: How?}}[-0cm]%<<<
...
are modelled. 
Section \ref{subsec:SedProfiles} compares modelled profiles with measured pore water data from different depths including the continental shelf (108\,m) and the lower slope (2213\,m) located at the Iberian margin,
the upper slope (585\,m) from the Santa Barbara Basin, and a deep sea site (4298\,m) in the Nazar\'e Canyon. 
Sediment characteristics and boundary conditions employed in OMEN-SED were set to the observed values where available and are shown in Table \ref{table:Profiles_BC}. 

The Santa Barbara Basin is characterised by anoxic bottom waters, high POC concentrations \citep[POC $\sim$5.5\,wt\%,][]{reimers_porewater_1996} and varved sediments \citep{reimers_seasonal_1990}, therefore the depth of bioturbation 
is restricted here to the upper 0.01 cm. 

\textit{Explanation later for high $H_2S$ in OMEN from Reimers (also check abstract): this maybe in discussion section: \\
...
sediments. In such surface sediments, a critical factor is that
...
The NW Iberian Margin ranks among the more productive areas of the global ocean (Longhurst, Sathyendranath, Platt, \& Caverhill, 1995)... The equatorward slope/shelf
...
\textbf{better fits could have been potentially acquired by using a sensitivity study over the C/N used in OMEN-SED: Epping: of showed C/N ratios exceeding Redfield stoichiometry}

For a more detailed description of the study areas and the experimental work, the interested reader is referred to the aforementioned papers. 

and , whereas the other sites at the Iberian margin (108 and 2213 m) and the Nazar\'e Canyon \citep[4298 m, ][]{epping_oxidation_2002} 
are oxic. 
Organic matter is modelled as two fractions, with different degradation rate constants. 
...
In section \ref{subsec:GENIE-pre-ind}, we test globally invariant values for the OM degradation rate constants ($k_1, k_2$) as well as the empirical relation of \citet{boudreau1997diagenetic} which is also used in 
Section \ref{subsec:globalhypsometry}. The modelled mean POC content in the upper 5cm of the sediments is compared to the global POC observations in surface sediments (< 5cm sediment depth) of \citet{seiter_organic_2004}. 
...
\begin{align}
  ...
  k_2 &= k_1/x
\end{align}
where $x$ describes the relation between $k_1$ and $k_2$ which is subject to sensitivity experiments in Section \ref{subsec:GENIE-pre-ind} (with values of $x \in \{2, 4, 5, 8, 10, 25, 50, 100, 125\}$). 
...
\section{Sediment profiles}\label{subsec:SedProfiles}
\textbf{needs more critical analysis\\
...
For the Santa Barbara basin (585 m) a misfit is observed for \chem{H_2S} and \chem{PO_4} in the upper 20 cm of the sediment. This can be explained by the presence of \chem{Mn^{2+}}, \chem{Fe^{2+}} and dissolved \chem{Fe} at 
this site which are either reduced to degrade POC and/or react with \chem{H_2S} to form iron sulfides, therefore inhibiting the rise in concentration of \chem{H_2S} \citep{reimers_porewater_1996}. 
...
Observations for \chem{O_2} and \chem{NO_3} fluxes are taken from \citet{middelburg_denitrification_1996}. Also plotted in Fig. \ref{fig:hypsometry}A are the total oxygen uptake (TOU) estimates presented in \citet{thullner_global_scale_2009} (filled red symbols), who assumed the organic matter flux to be equivalent to TOU. \\
\textbf{quickly describe what we see, i.e. fluxes decrease along the global hypsometry and mention why\\
also describe and explain  the differences between the OMEN min-max simulations}\\
Results show that OMEN-SED captures the main trends in observed TEA fluxes well. In general, fluxes calculated with BRNS fall within the range of possible OMEN-SED results. 
