...
and other model results \citep[e.g.][]{thullner_global_scale_2009} which indicate that the highest degradation rates in marine sediments are found in the coastal ocean (SFD < 200m). 
...
The new model OMEN-SED presented here is a practical and legitimate compromise between complexity of biogeochemical processes and computational efficiency. Its scope of applicability covers the entire range from regional to global scales. 
...
However, the model presented here, despite being more complete than previous analytical models, is inevitably associated with a certain degree of simplifications. In order to solve the diagenetic equation analytically important assumptions have been made, 
which limit the general applicability of the model. One of the most important simplifications is assuming steady-state. 
...
In future versions, non-steady state conditions could be approximated by implementing a relaxation between different steady states, similar to the schemes used in \citet{ruardij_benthic_1995} and \citet{arndt_model_2007}, 
thus enabling an application of the model to systems characterised by higher variability in boundary conditions. In addition, the unavoidable assumption of depth invariant porosity limits the correct calculation of the sediment-water interface flux 
of dissolved species as in reality porosity decreases with sediment depth. % \textbf{but how important is this??} \\

Furthermore, the global parameterisation of the sensitive model parameters (OM degradation rate constants and secondary redox parameters) is not straight forward. A comparison between simulated OM contents and observations indicates that 
depth dependent k-f relationships provide the best fit (Section \ref{subsubsec:couplingresultscrossplots}), confirming more theoretical considerations regarding the different time and reactivity scales that need to be considered 
(see Section \ref{subsec:Parameterising_OM_rate_const}). With respect to $\gamma$-values, model simulations along the global hypsometry (Section \ref{subsec:globalhypsometry}) have shown that high $\gamma$-values generally capture the main 
SWI-flux features, but have also highlighted that slightly lower $\gamma$-values would result in a better fit of SWI-fluxes to observations of the shallow ocean. 
In the future, fully formulated, multi-component diagenetic models could be used to derive transfer functions that would allow quantifying $\gamma$-values as a function of environmental forcings, such as organic matter degradation rates. 
...
Furthermore, the current version of OMEN-SED does not yet resolve the full range of important biogeochemical species, thus limiting its applicability.  Manganese and iron dynamics are not included and as such OMEN-SED is not able to 
simulate all processes important in coastal marine environments and highly accumulating upwelling regions. In particular the lack of iron limits the scope of applicability of the model, especially in anoxic systems characterized 
by high Fe oxide inputs, where redox driven iron dissolution-precipitation reactions induce a high internal recycling of the sedimentary Fe pool. This can cause problems when modelling \chem{H_2S} and \chem{PO_4} profiles in such environments 
as their concentrations are affected by these metal ions (compare Section \ref{subsec:SedProfiles}). Therefore, already planned future work will focus on an explicit description of iron. 
Finally, although the model also simulates DIC and alkalinity production and, thus, has the potential to predict pH profiles within the sediment, a major limitation at this stage is the lack of an explicit description for \chem{CaCO_3} 
precipitation/dissolution coupled to OM decomposition, which also controls the inorganic carbon system  \citep{krumins_dissolved_2013}. 
