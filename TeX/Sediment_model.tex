...
%Something on: Role of marine sediments for another time period , e.g. glacial/interglacial atmospheric p\chem{CO_2} cycles
...
These ``complete'', non-steady-state models, thus explicitly include oxic OM mineralisation, denitrification, oxidation by manganese and iron (hydr)oxides, sulphate reduction 
and methanogenesis as well as the reoxidation of reduced byproducts formed in the mineralization of OM (i.e. \chem{NH_4}, \chem{Mn^{2+}}, \chem{Fe^{2+}}, \chem{H_2S}, 
\chem{CH_4}). Furthermore, they incorporate various mineral dissolution and precipitation reactions, as well as fast equilibrium sorption processes for example of 
...
parameters \citep[see e.g.][for applications]{boudreau_comparative_1998, wang_multicomponent_1996, thullner_global-scale_2009}. 
...
to global ESMs (e.g. HAMOCC uses the model of \citet{heinze_global_1999}) or box models (e.g. DCESS, \citeauthor{shaffer_presentation_2008}, \citeyear{shaffer_presentation_2008}; 
MEDUSA, \citeauthor{munhoven_glacialinterglacial_2007}, \citeyear{munhoven_glacialinterglacial_2007}). 
These analytic or semi-analytical models include fewer biogeochemical reactions which are generally restricted to the upper, bioturbated 10\, cm of the sediments. 
The models assume that the sedimentary organic matter pool is composed 
of just a single compound class which is either degraded with a globally invariant degradation rate constant \citep[][]{munhoven_glacialinterglacial_2007} 
or a fixed rate constant depending on local oxygen concentrations \citep{shaffer_presentation_2008, palastanga_long-term_2011}. 
Pore water tracers explicitly represented in DCESS \citep{shaffer_presentation_2008} and the HAMOCC model of \citet{heinze_global_1999} and \citet{palastanga_long-term_2011} 
are only DIC, TA, \chem{PO_4} and \chem{O_2}. The MEDUSA model \citep{munhoven_glacialinterglacial_2007} considers \chem{CO_2}, \chem{HCO_3^-}, \chem{CO_3^{2-}} and \chem{O_2}. 
Thus, oxygen being the only TEA explicitly modelled and other species produced or consumed during respiration are neglected or the influence of reduced species is 
implicitly included in the boundary conditions for \chem{O_2}. 
A newer version of the HAMOCC model \citep{ilyina_global_2013} being a notable exception, as it includes \chem{NO_3} and denitrification explicitly. 
Secondary redox reactions involving reduced substances or sorption processes  are not included in any of the discussed models. 
...
Even though there is potential to use more appropriate global sediment representations, in most current ESMs sediment-water exchange of OM and chemical elements is either neglected or treated in a very 
...
and usually at least a part of the resulting characteristic redox-zonation of marine sediments \citep{soetaert_coupling_2000}. 
Examples of global carbon cycle models employing a vertically resolved diagenetic model are PISCES \citep{gehlen_reconciling_2006} (just offline coupling better 
discuss \citep{palastanga_long-term_2011} as example) and 
CLIMBER 3$\alpha$ \citep{ilyina_global_2013}, both using a version of \citet{heinze_global_1999}, however, just utilizing a 1G approach with a constant degradation rate constant. % and NORESM \citep{tjiputra_evaluation_2013} does not say anything if they just use the original oxic-only
None of the EMICs reviewed by \citet{huelse_biopump_models_2017} use such a sediment representation. 
\textcolor{red}{elaborate on the existing vertically resolved models and clearly highlight what are the key novel features of omensed}
...
that describes OM cycling as well as the associated dynamics of the most important TEAs (i.e. \chem{O_2}, \chem{NO_3}, \chem{SO_4}, \chem{CH_4}), 
...
\hline
Control & $1.0$ & $1.0$ & All OM is remineralised.\\
...
Palastanga & 0.01 & 0.008 & $D\leq 2000$\\
 & 0.005 & 0.002 & $D > 2000$ \citet{palastanga_long-term_2011} \\
\hline\hline
% inserts double horizontal lines
\end{tabular}
