...
if Nboot>1
    ...
    STi_all = nan(Nboot,M) ;
    for n=1:Nboot
        [ Si_all(n,:), STi_all(n,:) ] = compute_indices(YA(B(:,n)),YB(B(:,n)),YC(B(:,n),:))   ;
    end
    Si      = mean(Si_all) ;
    Si_sd   = std(Si_all)  ;
    ...
    [ Si, STi ] = compute_indices(YA,YB,YC)   ;   
...
function [ Si, STi ] = compute_indices(YA,YB,YC)
%
%    YA - vector (N,1) %
%    YB - vector (N,1) %
%    YC - matrix (N,M) %
