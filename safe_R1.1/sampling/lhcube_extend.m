...
%
% Output:
%   X_new = best latin hypercube                          - matrix(N_new,M)
...
if N_new < N ; error('''N_new'' must be larger than ''N''');end
...
for i=1:nrep
    % Xext=lhsdesign(N_new-N,M); % requires Matlab Statistical Toolbox
    Xext=lhcube(N_new-N,M); 
    Xi=[X;Xext];
    % dd=min(pdist(Xi)); % requires Matlab Statistical Toolbox
