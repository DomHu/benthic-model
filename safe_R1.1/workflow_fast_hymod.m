...
for n=2:length(NNfast)
    ...
    Yn = model_evaluation(myfun,Xn,rain,evap,flow) ;
...
% Run the model and compute model output at sampled parameter sets:
YA = model_evaluation(myfun,XA,rain,evap,flow) ;
YB = model_evaluation(myfun,XB,rain,evap,flow) ;
YC = model_evaluation(myfun,XC,rain,evap,flow) ;
