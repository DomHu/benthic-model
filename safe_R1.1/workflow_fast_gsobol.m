...
addpath([ my_dir '/vbsa'])
addpath([ my_dir '/FAST'])
addpath([ my_dir '/example/sobol_g_function'])
...
for param = 1:4
    ...
    for m=1:length(MM)
        ...
        % sets:
        Y = model_evaluation(myfun,X,a) ; % size (Ns,1)
   ...
   %     Nvbsa(m) = 4096 ;
        ...
        YA = model_evaluation(myfun,XA,a) ; % size (N,1)
        YB = model_evaluation(myfun,XB,a) ; % size (N,1)
        YC = model_evaluation(myfun,XC,a) ; % size (N*M,1)
