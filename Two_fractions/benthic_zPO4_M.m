classdef benthic_zPO4_M
    ...
    properties                      
        ...
        ksPO4=3.65; %1.0; %0.26*365;      %Rate constant for kinetic P sorption (1/yr)   0.12 fits 1.CASE; 2.2 fits 2. CASE DOM: was 0.5*365 from Nicolas; Slomp ea 1996 0.26
       % ksPO4=1e-15;
        %kmPO4= 1e-15 ;
        kmPO4=0.19; %2.2e-6*24*365;          % Dom was from Slomp 0.00053*365;	%Rate constant for Fe-bound P release upon Fe oxide reduction   DOM: was 1.8e-6 Slomp ea 1996 0.00053*365 
        %kaPO4 = 0.0;
        kaPO4=0.37;% 10.0; % Dom was 0.001*365;	%Rate constant for authigenic P formation (1/yr)    DOM: was 0.004*365 from Nicolas; Slomp ea 1996 0.001
        PO4s=12.0e-9;        %Equilibrium concentration for P sorption (mol/cm3)       was 1.5e-9; ; Slomp ea 1996
        PO4a= 3.7e-9; %9.0e-8; %47e-9;  %was 3.7e-9      %Equilibrium concentration for authigenic P formation (mol/cm3) was 0.7e-9
        %Minf = 0;
        Minf=5.2e-9; %2.0e-9; %1.0e-10;       % asymptotic concentration for Fe-bound P (mol/cm3)      TODO/CHECK: good value? is from Slomp et al. 1996 Dom was 1.99e-6
...
%        FeSCt=FePdeepConc*(1.-por);                                 %convert FePdeepConc from /solid to /bulk
    ...
    methods
        function obj = benthic_zPO4_M(bsd, swi)
            ...
            %DH 2405 was obj.DPO41=((obj.qdispPO4+obj.adispPO4*swi.T).*bsd.dispFactor+bsd.Dbio)/(1+obj.KPO4_ox);            %PO4 diffusion coefficient in bioturbated layer (cm2/yr)
            %DH 2405 was obj.DPO42=((obj.qdispPO4+obj.adispPO4*swi.T).*bsd.dispFactor)/(1+obj.KPO4_anox);                     %PO4 diffusion coefficient in non-bioturbated layer (cm2/yr)
